...
export const ReceiptPrinter = (() => {
  ...
  const iconv = (() => {
    ...
    while (true) {
      for (let i = 0x30; i <= 0x39; i++) {
        if (
          !(
            (f === 0x81 && g === 0x39 && h === 0xee && i < 0x39) ||
            (f === 0x82 && g === 0x35 && h === 0x87 && i > 0x38)
          )
        ) {
  ...
  const cp1252Extra = new Set([
    0x20ac, 0x201a, 0x0192, 0x201e, 0x2026, 0x2020, 0x2021, 0x02c6, 0x2030,
    0x0160, 0x2039, 0x0152, 0x017d, 0x2018, 0x2019, 0x201c, 0x201d, 0x2022,
    0x2013, 0x2014, 0x02dc, 0x2122, 0x0161, 0x203a, 0x0153, 0x017e, 0x0178,
  ]);

  const isCp1252CodePoint = (d) => {
    if (d < 0x80) return true;
    if (d >= 0xa0 && d <= 0xff) return true;
    return cp1252Extra.has(d);
  };

  const autoLength = (text) =>
    Array.from(text).reduce(
      (a, c) => a + (isCp1252CodePoint(c.codePointAt(0)) ? 1 : 2),
      0,
    );

  const encodeAuto = (text, codepage) => {
    let r = "";
    let current = null;
    let cjkMode = false;
    const gbCodepage = codepage.gb18030 || "";
    const hasGbFsAnd = gbCodepage.includes("\x1c&");
    for (const c of Array.from(text)) {
      const enc = isCp1252CodePoint(c.codePointAt(0)) ? "cp1252" : "gb18030";
      if (enc !== current) {
        // Exit CJK mode when switching to cp1252 (some printers keep FS & until FS .)
        if (enc === "cp1252" && cjkMode) {
          r += "\x1c.";
          cjkMode = false;
        }
        if (enc === "gb18030") {
          r += gbCodepage || "";
          if (!hasGbFsAnd) {
            // Ensure CJK mode is on if codepage string doesn't include FS &
            r += "\x1c&";
          }
          cjkMode = true;
        } else {
      ...
      r += iconv.encode(c, enc).toString("binary");
    }
    if (cjkMode) {
      r += "\x1c.";
    }
  ...
  const _thermal = {
    ...
    open(printer) {
      ...
      const fsA = printer.encoding === "auto" ? "" : "\x1c(A" + $(2, 0, 48, 0);
      return (
        "\x1b@\x1da\x00\x1bM" +
    ...
    text(text, encoding) {
      switch (encoding) {
        case "auto":
          return encodeAuto(text, this.codepage);
        case "multilingual":
  ...
  const _impact = {
    ...
    text(text, encoding) {
      ...
      let l = 0;
      if (encoding === "auto") {
        d = "\x1b!" + $(this.style) + encodeAuto(text, this.codepage);
        l = autoLength(text) * (this.style & 32 ? 2 : 1);
      } else {
        const t = iconv
  ...
  const _escpos90 = {
    ...
    text(text, encoding) {
      switch (encoding) {
        case "auto":
          this.content += encodeAuto(text, this.codepage);
          break;
        case "multilingual":
    ...
    barcode(symbol, encoding) {
      ...
      if ("length" in bar) {
        const w =
          bar.length +
          symbol.width *
            (/^(upc|[ej]an)$/.test(symbol.type)
              ? symbol.data.length < 9
                ? 14
                : 18
              : 20);
        const l = symbol.height;
