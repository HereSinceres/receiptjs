...
export const Receipt = (() => {
        ...
        if (/^\{[^{}]*\}$/.test(element)) {
          ...
          if (array.length === 1) {
            ...
            if ("image" in result.property) {
              const c = result.property.image.replace(
                /=.*|[^A-Za-z0-9+/]/g,
                "",
              );
              switch (c.length % 4) {
  ...
  const barcode = (() => {
    ...
    const upce = (symbol) => {
      ...
      if (d.length > 0) {
        ...
        d[7] =
          (10 -
            (upcetoa(d).reduce((a, c, i) => a + c * (3 - (i % 2) * 2), 0) %
              10)) %
          10;
        // generate HRI
        r.hri = symbol.hri;
  ...
  const qrcode = (() => {
    ...
    const createData = (utf8, level, version) => {
      ...
      const d =
        version < 10 ? [mode, len, ...utf8] : [mode, len >> 8, len, ...utf8];
      const data = new Uint8Array(getCapacity(level, version) + d.length - len);
      data.set(
        d.map((c, i) => (c << 4) | ((i < d.length - 1 ? d[i + 1] : 0) >> 4)),
      );
    ...
    const evaluateMask = (m) => {
      ...
      score +=
        Math.ceil(Math.abs((d * 100) / (size * size) - 50) / 5) * 10 - 10;
      return score;
    };
      ...
      switch (encoding) {
        case "auto":
          r = t.reduce((a, c) => {
            ...
            return a + (isCp1252CodePoint(d) ? 1 : 2);
          }, 0);
          break;
        case "cp932":
        case "shiftjis":
  ...
  const _svg = {
    ...
    lf() {
      ...
      if (this.textElement.length > 0) {
        this.svgContent += `<g transform="translate(${this.lineMargin * this.charWidth},${
          this.svgHeight + h
        })">${this.textElement}</g>`;
      }
    ...
    barcode(symbol, encoding) {
      ...
      if ("length" in bar) {
        ...
        if (bar.hri) {
          const m = Math.floor(
            (width -
              (this.measureText(bar.text, encoding) - 1) * this.charWidth) /
              2,
          );
      ...
      if (!/^(ko|zh)/.test(lang)) {
        ...
        group.querySelectorAll("g").forEach((g) => {
          ...
          g.querySelectorAll("text").forEach((text) => {
            ...
            const [sx, sy] = text.hasAttribute("transform")
              ? text
                  .getAttribute("transform")
                  .match(/scale\((\d),(\d)\)/)
                  .slice(1)
            ...
            text.querySelectorAll("tspan").forEach((tspan) => {
              ...
              const w =
                c *
                commands.svg.measureText(t, encoding[lang] || "multilingual");
              const h = c + c;
  ...
  const parseOption = (options) => {
    ...
    if (
      !/^(escpos|epson|sii|citizen|fit|impactb?|generic|star(line|graphic|impact[23]?)?|emustarline)$/.test(
        p,
      )
    ) {
      p = "base";
  ...
  return {
    ...
    from(markdown, options) {
      ...
      return {
         ...
         */
        async toText() {
          return await transform(markdown, {
            ...params,
        ...
        async toCommand() {
          ...
          // print as image
          if (params.asImage) {
            const r90 = params.landscape
              ? { cpl: 48, margin: 0, marginRight: 0 }
              : {};
            const png = await Base64PNG.from(await this.toSVG(), params);
            return await transform(`|{i:${png.replace(/^data:.*,/, "")}}`, {
              ...params,
              ...r90,
              ...{ command: ReceiptPrinter.create(p) },
            });
          }
          // print with device font
          if (
            params.landscape &&
            /^(escpos|epson|sii|citizen|star[sm]bcs2?)$/.test(p)
          ) {
            p += "90";
          }
