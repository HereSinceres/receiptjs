    ...
    connect(options = {}) {
      ...
      conn.on("open", async () => {
        ...
        timeout = setTimeout(async () => {
          // buffer clear
          drain = await conn.write(
            "\x00".repeat(65536) + command.hello,
            "binary",
          );
          // set timer
          timeout = setTimeout(async () => {
          ...
          switch (printer) {
            case "":
              if ((buffer[0] & 0xf0) === 0xb0) {
                ...
                if (len > 1) {
                  const l =
                    (((buffer[0] >> 2) & 0x18) | ((buffer[0] >> 1) & 0x07)) +
                    ((buffer[1] >> 6) & 0x02);
                  // check length
                  if (l <= len) {
                    // printer
            ...
            case "star":
              if ((buffer[0] & 0xf1) === 0x21) {
                ...
                if (len > 1) {
                  const l =
                    (((buffer[0] >> 2) & 0x08) | ((buffer[0] >> 1) & 0x07)) +
                    ((buffer[1] >> 6) & 0x02);
                  // check length
                  if (l <= len) {
                    // printer
                ...
                if (status !== state.offline) {
                  ...
                  timeout = setTimeout(async () => {
                    ...
                    timeout = setTimeout(async () => {
                      // buffer clear
                      drain = await conn.write(
                        "\x00".repeat(65536) + command.escclr,
                        "binary",
                      );
                    }, 3000);
                  }, 3000);
      ...
      // open port
      conn.open({
        ...{ baudRate: 115200, flowControl: "hardware", bufferSize: 2048 },
        ...options,
      });
        ...
        print(markdown, options) {
          ...
          return new Promise(async (res) => {
            ...
            if (status === state.online) {
              ...
              const command = await Receipt.from(
                markdown,
                `-p ${printer} ${options}`,
              ).toCommand();
              // write command
              if (/^star$/.test(printer)) {
                ...
                drain = await conn.write(
                  command
                    ...
                    .replace(
                      /(\x1b\x1d\x03\x01\x00\x00\x04?|\x1b\x06\x01)$/,
                      "\x17",
                    ),
                  "binary",
                ); // ETB
                ...
                // escpos
                drain = await conn.write(
                  command.replace(/^\x1b@\x1da\x00/, "\x1b@\x1da\xff"),
                  "binary",
                ); // ESC @ GS a n
              }
            } else {
